...
def crear_caso(
    ...
    (ignora el responsable_id del request si se envía).
    """
    logger.info(f"Creando caso - Usuario: {current_user.email}")
    
    # Crear caso - siempre asignar al usuario actual como responsable
    caso = Caso(
        titulo=data.titulo,
        estado=data.estado,
        prioridad=data.prioridad,
        fecha_vencimiento=data.fecha_vencimiento,
        responsable_id=current_user.id,  # Siempre el usuario actual
