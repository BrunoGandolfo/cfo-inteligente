...
export function OperationRow({ op, onSelect, onEdit, onDelete }) {
  ...
  return (
    <tr className="hover:bg-surface-alt cursor-pointer" onClick={() => onSelect(op)}>
      <td className="px-4 py-3 text-sm text-text-primary">{formatDateShort(op.fecha)}</td>
      <td className="px-4 py-3"><Badge tipo={tipo} /></td>
      <td className="px-4 py-3 text-sm text-text-primary">{op.cliente || op.proveedor || '-'}</td>
      <td className="px-4 py-3 text-sm text-right font-semibold text-text-primary">{formatMoneySmart(op)}</td>
